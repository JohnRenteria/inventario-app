...
const ActivateUserModal = ({ user, isOpen, onClose }) => {
  ...
  const handleActivate = async () => {
    ...
    try {
      ...
      showModal({ title: 'Éxito', message: 'Usuario reactivado y rol actualizado con éxito.' });
      onClose();
    } catch (err) {
      console.error("Error al reactivar el usuario:", err);
      showModal({ title: 'Error', message: 'Ocurrió un error al reactivar el usuario.' });
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={`Reactivar a ${user?.nombre}`} footer={modalFooter}>
      <div className="input-group" style={{ padding: '0 1rem' }}>
        ...
        <select value={newRol} onChange={(e) => setNewRol(e.target.value)}>
          <option value="" disabled>Selecciona un rol disponible</option>
          {/* Si el rol actual del usuario a reactivar está disponible, se muestra normalmente.
              Si no, se muestra como una opción especial para poder reasignárselo. */}
          {user && !availableRoles[user.rol] &&
            <option key={user.rol} value={user.rol}>{user.rol} (Rol actual)</option>}
